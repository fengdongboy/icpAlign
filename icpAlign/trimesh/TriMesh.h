...
public:
	...
	//wzk
	//::std::vector<bool> bKeyFrame;
	//OriginalKeyFrame 原始帧序号 这个点有哪帧里提取的
	::std::vector<int> OriginalKeyFrame;
	::std::vector<int> KeyFrameIndex;
	//KeyFrameIndex 这帧被哪帧关键帧所涵盖了 KeyFrameIndex初始值为-1 如果他被关键帧涵盖了， 则将它改为变为那帧关键帧的序号
	//::std::vector<int> KeyFrameIndex;
...
struct renderingData
{
	...
	bool write(const std::string& name) const
	{
		...
		fwrite(&idxs[0], 1, sizeof(int) * n, f);
		fwrite(&points[0], 1, sizeof(point)*n, f);
		fwrite(&norms[0], 1, sizeof(vec)*n, f);
		fclose(f);
		return true;
	}
	...
	bool read(const std::string& name)
	{
		...
		points.resize(n);
		norms.resize(n);
		fread((void *)&idxs[0], 1, sizeof(int) * n, f);
		fread((void *)&points[0], 1, sizeof(point)*n, f);
		fread((void *)&norms[0], 1, sizeof(vec)*n, f);
		fclose(f);
		return true;
	}
};
