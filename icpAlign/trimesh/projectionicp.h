...
namespace trimesh
{
	...
	public:
		...
		void SetLeastOverlapRatio(float least_overlap_ratio);
		void SetSource(trimesh::TriMesh* source);
		void SetTarget(trimesh::TriMesh* target);
		void SetMappingType(int mapping_type);
		void SetFixType(float* k2);
	...
	private:
		...
		float FastInteration_F2F(trimesh::xform &source_form);
